...
-(void)updateUI{
    if([[self.history firstObject] isKindOfClass:[NSAttributedString class]]){
        ...
        for(NSAttributedString *item in self.history){
            [historyText appendAttributedString:[[NSAttributedString alloc] initWithString:[NSString stringWithFormat:@"-----STEP %2ld:-----\n", count++]]];
        ...
        for(NSString *item in self.history) {
            historyText = [NSString stringWithFormat:@"%@-----STEP %2ld:-----\n%@\n\n",historyText,count++,item];
        }
